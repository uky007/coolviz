...
| `docs/okinawa.png`, `docs/okinawa_sea.png`, `docs/okinawa_sim.mp4` | Fully procedural render, no external data | Same terms as the code (MIT) |
| `docs/tokyo_flood*.png`, `docs/tokyo_flood*.mp4` | Rendered by coolviz; building geometry from **Project PLATEAU** (国土交通省, 2020), terrain from **国土地理院 標高タイル** (dem5a), road network © OpenStreetMap contributors (ODbL) | Attribution: "Source: 国土交通省 Project PLATEAU / 国土地理院 / © OpenStreetMap contributors" |
...
(CC BY 4.0-compatible, cached under `.cache/plateau/`), GSI elevation tiles
(出典明記, cached under `.cache/gsi/`), Overpass/OpenStreetMap roads (ODbL),
and the JMA high-resolution precipitation nowcast tiles (**unofficial endpoint** — the
